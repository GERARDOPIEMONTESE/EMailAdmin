...
{
    public class EmailSenderQuoteExchange : EMailSenderDefault
    {
        protected override void CompleteDto(DTO.AbstractEMailDTO dto)
        {
            QuoteExchangeDTO quoteExchangeDto = (QuoteExchangeDTO)dto;

            quoteExchangeDto.MailFromAppearance = ConfigurationManager.AppSettings["QuoteExchangeMailFromAppearance"].ToString(); //"pruebadecumples@assist-card.com";
            quoteExchangeDto.TemplateType = TemplateTypeHome.Get("QE");
            quoteExchangeDto.HtmlBody = QuoteExchangeBodyHome.GetBody().Body;
            dto.Bcc = ConfigurationManager.AppSettings["QuoteExchangeTo"].ToString();
        }
        ...
        {
            string xml = SerializeObject((QuoteExchangeDTO)dto);
