...
{
    public class EMailSenderConditionAlert : EMailSenderDefault
    {
        ...
        protected override void CompleteDto(AbstractEMailDTO dto)
        {
            ...
            dto.TemplateType = TemplateTypeHome.Get("CAT"); // "Condition Alert Template"
        }
        ...
        protected override void Proccess(AbstractEMailDTO dto, Template template, EMailLog log)
        {
            ...
            try
            {
                // Enviar correo solo si hay vouchers por expirar
                int count = DAOLocator.Instance().GetDAOVoucherACNETDTO().VoucherPointsToExpire();
                if (count > 0)
                {
                    if (template.Id != 0)
                    {
